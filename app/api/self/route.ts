...
const compareMoonToPlanets = (
  ...
  const saturnAngle = angularDistance(
    moonToday.longitude,
    natalSaturn.longitude
  );
...
const determineSignalStrength = (moonAspect: {
  planet: "Saturn" | "Mars" | null;
  aspectType: string | null;
}): SelfSignalStrength => {
...
const buildLogicalBullets = (
  ...
  if (moonAspect.planet === "Saturn") {
    bullets.push(
      "Moon is activating Saturn's themes of restraint and emotional weight"
    );
  ...
  if (moonAspect.planet === "Mars") {
    bullets.push(
      "Moon is activating Mars's themes of reactivity and defensiveness"
    );
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          return {
            summary:
              parsed.summary ||
              "Your emotional skin feels more sensitive today.",
            explanation:
              parsed.explanation ||
              "The Moon's activation brings attention to your inner emotional state.",
            encouragement:
              parsed.encouragement ||
              "Be gentle with yourself and honor your need for care and awareness.",
          };
        }
      } catch (e) {
...
export async function GET(req: Request) {
  try {
    ...
    const cached = await getCachedGeneration(dbUser.id, "self", today);

    if (cached) {
      return NextResponse.json({ self: cached });
    }
    ...
    await setCachedGeneration(dbUser.id, "self", today, selfData);
